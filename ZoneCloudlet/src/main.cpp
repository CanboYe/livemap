...
int main(int argc, char *argv[])
{
	...
	std::string CLIENT_ID2("async_subcribe_cpp2");
	std::string TOPIC("HAZARDS_DETECTED");
	std::string TOPIC2("DRIVE");
	int QOS = 1;
	...
	cb.setCloudlet(cloudlet);
	client.set_callback(cb);


	mqtt::async_client client2(SERVER_ADDRESS, CLIENT_ID2);

	driveCompleteCallback cb2(client2, connOpts, SERVER_ADDRESS, CLIENT_ID, TOPIC2, QOS, N_RETRY_ATTEMPTS);
	cb2.setCloudlet(cloudlet);
	...
	try
	{
		...
		client2.connect(connOpts, nullptr, cb2);
	}
