...
void Utils::makeDBConfigFile(const std::string &dbname , 
                             ...
                             const std::string &port)
{
    ...
    d.AddMember("dbname",  dbname, allocator);
    d.AddMember("user",  user , allocator);
    d.AddMember("password", password , allocator);
    d.AddMember("hostaddr", hostaddr , allocator);
    d.AddMember("port", port, allocator);
    ...
    std::string config = strbuf.GetString();
    // Write the config to file
    std::ofstream filename("../config/DatabaseConfig.JSON",std::ofstream::out);
    filename << config;
    filename.close();
